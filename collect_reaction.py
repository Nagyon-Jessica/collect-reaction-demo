...
def change_time_JST(u_time):
    ...
    jst_time_until = jst_time_since + timedelta(minutes=10)
    # 文字列で返す
    str_time_since = jst_time_since.strftime("%Y-%m-%d_%H:%M:%S_JST")
    str_time_until = jst_time_until.strftime("%Y-%m-%d_%H:%M:%S_JST")
    return (str_time_since, str_time_until)
...
while True:
    ...
    for tweet in ret:
        ...
        user = tweet.user
        # 検索範囲（開始，終了）
        since, until = change_time_JST(tweet.created_at)
        # 検索クエリ
